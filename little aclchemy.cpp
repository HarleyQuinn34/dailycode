...
int main()
{
	...
	ALLEGRO_BITMAP *dpPic = al_load_bitmap("dp.jpg");
	element air;
	element water;
	element fire;
	element earth;
	...
	dp.initElement(500, 500, 5, dpPic);
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (fire.clicked(bouncer_x, bouncer_y))
				if (fire.isLifted() == false)
					...
					fire.PutDown();
			if (dp.clicked(bouncer_x, bouncer_y))
				if (dp.isLifted() == false)
					dp.liftUp();
				else
					dp.PutDown();
		}
